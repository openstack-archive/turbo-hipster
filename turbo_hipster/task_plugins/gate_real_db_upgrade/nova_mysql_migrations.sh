...
EOF
  ...
  ls $3/nova/db/sqlalchemy/migrate_repo/versions/*.py
  ...
  if [ "%$8%" == "%%" ]
  then
    end_version=`ls $3/nova/db/sqlalchemy/migrate_repo/versions/*.py | egrep "^[0-9]+_" | tail -1 | cut -f 1 -d "_"`
  else
    end_version=$8
  fi
  ...
  for i in `seq $(( $start_version + 1 )) $increment $end_version`
  do
    set -x
    sudo /sbin/ip netns exec nonet `dirname $0`/nova-manage-wrapper $VENV_PATH --config-file $2/nova-$1.conf --verbose db sync $8 --version $i
    manage_exit=$?
    set +x
